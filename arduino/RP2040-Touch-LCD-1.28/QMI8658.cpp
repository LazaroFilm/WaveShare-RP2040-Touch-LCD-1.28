...
void QMI8658_config_ae(enum QMI8658_AeOdr odr) {
  ...
  QMI8658_write_reg(QMI8658Register_Ctrl6, odr);  // AE odr
}
...
void QMI8658_read_ae(float quat[4], float velocity[3], unsigned int *tim_count) {
  ...
  Serial.print(" | quat_r_w = ");
  Serial.print(raw_q_xyz[0]);
  Serial.print(" | quat_r_x = ");
  Serial.print(raw_q_xyz[1]);
  Serial.print(" | quat_r_y = ");
  Serial.print(raw_q_xyz[2]);
  Serial.print(" | quat_r_z = ");
  Serial.println(raw_q_xyz[3]);
...
unsigned char QMI8658_init(void) {
  ...
  if (QMI8658_chip_id == 0x05) {
    ...
    QMI8658_config.inputSelection = QMI8658_CONFIG_AE_ENABLE;  // QMI8658_CONFIG_ACCGYR_ENABLE;
